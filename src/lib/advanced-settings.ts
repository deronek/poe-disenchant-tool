 ...
 */
export function migrateLegacyAdvancedSettings(
  storage: Storage,
): AdvancedSettings | null {
  const legacyRaw = storage.getItem(LEGACY_ADVANCED_SETTINGS_STORAGE_KEY_V1);
  if (legacyRaw === null) return null;

  storage.removeItem(LEGACY_ADVANCED_SETTINGS_STORAGE_KEY_V1);
  ...
  try {
    ...
    storage.setItem(ADVANCED_SETTINGS_STORAGE_KEY, JSON.stringify(migrated));
    return migrated;
  } catch {
    return null;
  }
